...
public class OverlayDemo extends Activity {
	...
	public void initOverlay() {
		...
		OverlayOptions ooText = new TextOptions().bgColor(0xAAFFFF00).fontSize(24).fontColor(0xFFFF00FF).text("K117").rotate(0).position(llA);
		mBaiduMap.addOverlay(ooText);//添加文字
		...
		for (int i = 0; i < 3; i++) {
			a = a + d;
			llD2 = new LatLng(a, b);
			ooD = new MarkerOptions().position(llD2).icon(bdD).perspective(false).zIndex(7);
			mMarkerD = (Marker)mBaiduMap.addOverlay(ooD);
